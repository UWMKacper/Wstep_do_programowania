/* Cwiczenie_1.c - Modyfikacja programu meteorologicznego z listingu 10.7, tak
aby wykonywal obliczenia, korzystajac ze wskaznikow zamiast indeksow. */
...
int main(void)
{
    ...
    for (rok = 0, suma = 0; rok < LATA; rok++)
        ...
        for (miesiac = 0, podsuma = 0; miesiac < MIESIACE; miesiac++)
            ...
            podsuma += *(*(deszcz + rok) + miesiac);  // *(deszcz + rok) == deszcz[0+rok] | *(*(deszcz + rok) + miesiac) == deszcz[0+rok][0+miesiac]
        printf("%5d %12.1f\n", 2000 + rok, podsuma);
